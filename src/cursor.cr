...
class Mongo::Cursor
  ...
  def next
    ...
    if @cursor_id == 0
      ...
      reply = @client.command(Commands::GetMore, database: @database, collection: @collection, cursor_id: @cursor_id, max_time_ms: @await_time_ms).not_nil!
  ...
  def close
    @mutex.synchronize {
      unless @closed || @cursor_id == 0
        @client.command(Commands::KillCursors, database: @database, collection: @collection, cursor_ids: [@cursor_id])
        @closed = true
      end
    }
  end
